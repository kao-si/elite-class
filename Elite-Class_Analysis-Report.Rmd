...
## Elite students and top regular students by subgroups of HSEE total score

```{r}
gg_bin <- function(cohort_val, bin_var) {
    lab <- dat %>%
      filter(!is.na(top_cat), exam == "hsee", cohort == cohort_val) %>%
      group_by(track, top_cat, !!rlang::sym(bin_var)) %>%
      # Compute counts and groupwise max(tot) per bin within each facet group (track × top_cat)
      summarize(
        n_bin = n(),
    ...
    dat %>%
      filter(!is.na(top_cat), exam == "hsee", cohort == cohort_val) %>%
      ggplot(aes(
        x = factor(!!rlang::sym(bin_var)),
        y = tot,
        color = factor(!!rlang::sym(bin_var))
      )) +
        geom_point(size = 1, alpha = 0.5,
          position = position_jitter(width = 0.2, height = 0)) +
        # Add count labels (one per bin per facet)
        geom_text(
          data = lab,
          aes(
            x = factor(!!rlang::sym(bin_var)),
        ...
        guides(color = "none") +
        labs(
          title = paste0("HSEE Total Scores by Subgroups in Cohort ", cohort_val, " using ", bin_var),
          x = "Bin",
          y = "HSEE Total Score"
        )
}
...
# Print all plots
vec_cohort_val <- rep(unique(dat$cohort), each = 3)

vec_bin_vars <- rep(c("hsee_decile", "hsee_2decile", "hsee_5decile"), times = length(unique(dat$cohort)))

walk2(vec_cohort_val, vec_bin_vars,  ~ print(gg_bin(.x, .y)))
```
