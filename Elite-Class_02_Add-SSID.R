...
repmis <- function(df, xjh = "ssid", check, dfxjh, fxjh) {
  ...
  # Filter rows in 'check' where 'dfxjh' does not match 'fxjh'
  misdf <- dplyr::filter(check, !!rlang::sym(dfxjh) != !!rlang::sym(fxjh))

  # Update 'df[[xjh]]' with the correct values from 'misdf[[fxjh]]'
...
fillna <- function(df, uiv = "cls_name") {
  ...
  # n + 2 is the position of XJH extracted from the second source file and so on
